...
class CommentSpider(Spider):
    ...
    def start_requests(self):
        ...
        tweet_ids = get_list_config('WEIBO_TWEET_IDS', 'tweet_ids.txt', ['Mb15BDYR0'])
        for tweet_id in tweet_ids:
            ...
            yield Request(url, callback=self.parse, meta={'source_url': url, 'root_tweet_id': tweet_id})
    ...
    def parse(self, response, **kwargs):
        ...
        root_tweet_id = response.meta.get('root_tweet_id')
        for comment_info in data['data']:
            item = self.parse_comment(comment_info, root_tweet_id)
            yield item
            # 解析二级评论
            if 'more_info' in comment_info:
                ...
                yield Request(
                    ...
                    callback=self.parse,
                    priority=20,
                    meta={'source_url': response.meta.get('source_url', url), 'root_tweet_id': root_tweet_id},
                )
    ...
    @staticmethod
    def parse_comment(data, root_tweet_id=None):
        """
        解析comment
        """
        item = dict()
        ...
        if root_tweet_id:
            item['root_tweet_id'] = root_tweet_id
